...
nodes(1) = q_start;
    ...
    if isCollisionC(q_new.coord,obs)
    ...
    neighbor_count = 1;
    for j = 1:1:length(nodes)
        if  isCollisionC(q_new.coord,obs) && (dist_3d(nodes(j).coord, q_new.coord)) <= r
    ...
    for k = 1:1:length(q_nearest)
        if isCollisionC(q_new.coord,obs) && q_nearest(k).cost + dist_3d(q_nearest(k).coord, q_new.coord) < C_min
            q_min = q_nearest(k);
