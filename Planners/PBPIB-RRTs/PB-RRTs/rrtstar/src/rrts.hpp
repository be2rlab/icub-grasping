...
::rewireVertices (Vertex<State,Trajectory,System>& vertexNew, std::vector< Vertex<State,Trajectory,System>* >& vectorNearVertices,int a) {
    ...
    //Counter for counting number of re-wires happening
    int counter = 0;
...
int 
RRTstar::Planner<State, Trajectory, System>
::iteration (int k,int &rewire_counter, int current_cycle) {
    
int chexk;
bool flag_evensample = false; 
State stateRandom;  
    if(k % 2 == 0){

    flag_evensample = true;
    //State stateRandom;
    system->sampleState_i_to_g (stateRandom,k,current_cycle); //itog stands for initial to goal
    }else{

	  flag_evensample = false;
	  //State stateRandom;
          //system->sampleState (stateRandom);
          system->sampleState_g_to_i (stateRandom,k,current_cycle); //itog stands for initial to goal
    } 

    
   /* //For even sample run potential fields directed towards Goal 
    if(flag_evensample == true){
    // 1. Sample a new state

    State stateRandom;
    //system->sampleState (stateRandom);
    system->sampleState_i_to_g (stateRandom); //itog stands for initial to goal
    }else{ 
	   //For odd sample run potential fields directed towards intial pos   
	   // 1. Sample a new state

           //State stateRandom;            
           system->sampleState_g_to_i (stateRandom); //gtoi stands for goal to initial 
	}*/
   ...
   // If input k is even then potentially guide tree A towards Goal!!   
   if(flag_evensample == true){
    ...
    if(flagA==true && flagB==true)
      ...
      }else if (flagA == false && flagB == true)
     {
        //insert into tree A
         ...
         return 0;
         if (vectorNearVertices.size() > 0) 
         rewire_counter = rewireVertices (*vertexNew, vectorNearVertices,1);         
...
//        rewireVertices (*vertexNew, vectorNearVertices,1);
	...
	if (vectorNearVertices2.size() == 0) {
	 ...
	 // 3.b Extend the best parent within the near vertices
        if (findBestParent (stateRandom, vectorNearVertices2, vertexParent2, trajectory2, exactConnection2) <= 0)         
            return 0;//{flagB=true;goto y;}//return 0;
        }
    ...
    if(flagA==true && flagB==true)
      ...
      }else if (flagB == false && flagA == true)
     {
        //insert into treeB
        Vertex<State,Trajectory,System>* vertexNew = insertTrajectory (*vertexParent2, trajectory2,2);
         ...
         return 0;
         if (vectorNearVertices2.size() > 0) 
         rewire_counter = rewireVertices (*vertexNew, vectorNearVertices2,2);
...
::getBestTrajectory (std::list<double*>& trajectoryOut, int a) {
    ...
    while (vertexCurr) {
        ...
        double *stateArrCurr = new double[3];  //new double[2]; 
        stateArrCurr[0] = stateCurr[0];
        stateArrCurr[1] = stateCurr[1];
        stateArrCurr[2] = stateCurr[2];

	/*double *stateArrCurr = new double[2];  //new double[2]; 
        stateArrCurr[0] = stateCurr[0];
        stateArrCurr[1] = stateCurr[1];*/
